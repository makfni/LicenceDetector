...
file = 'Images/plate5.jpg'

img = cv2.imread(file)
# img = cv2.resize(img, (520, 380))
img = cv2.resize(img, (620, 480))
gray = cv2.imread(file, 0)
# gray = cv2.resize(gray, (520, 380))
gray = cv2.resize(gray, (620, 480))

gray = cv2.bilateralFilter(gray, 11, 17, 17)  # Blur to reduce noise while preserving edges
...
def _my_edge_filter(img0, sigma):
    ...
    #the 4 extreme point coordinates (x,y)
    rc = cv2.minAreaRect(screenCnt)
